...
def main():
    ...
    if fine_tune.lower() in ["true", "1", "t", "y", "yes"]:
        ...
        opt_ft = keras.optimizers.RMSprop(learning_rate=0.000001 * hvd.size())
        opt_ft = hvd.DistributedOptimizer(opt_ft)

        # Compile the model with a low learning rate
        model.compile(
            optimizer=opt_ft, loss="categorical_crossentropy", metrics=["accuracy"]
        )
...
def save_loss_plot(history, args):
    ...
    plt.ylabel("Accuracy")
    plt.title(
        "Fruit Classification, Batch Size: "
        + args.batch_size
        + " Epochs: "
        + args.epochs
    )
